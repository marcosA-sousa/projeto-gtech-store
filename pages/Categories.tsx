      ...
      <div className="container mx-auto px-4 lg:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8">
          {CATEGORIES.map((cat, idx) => {
            const IconComp = (LucideIcons as any)[cat.icon] || LucideIcons.Package;
            return (
              <Link 
                key={idx} 
                to={`/produtos?categoria=${cat.name}`}
                className="group bg-white dark:bg-gray-900 rounded-[30px] p-10 shadow-sm border border-transparent dark:border-gray-800 hover:shadow-md transition-all flex flex-col items-center text-center relative overflow-hidden"
              >
                <div className="w-24 h-24 rounded-full bg-[#F5F5F5] dark:bg-gray-800 flex items-center justify-center mb-6 group-hover:bg-primary/5 transition-colors">
                  <IconComp className="w-10 h-10 text-[#8F9BB3] group-hover:text-primary transition-colors" />
                </div>
